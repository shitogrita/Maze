...
void MainWindow::SetupMazeTab() {
  ...
  connect(solve_button_, &QPushButton::clicked, this,
...
void MainWindow::SetupCaveTab() {
  ...
  connect(cave_timer_, &QTimer::timeout, this, &MainWindow::OnCaveTimerTimeout);
  ...
  connect(cave_generate_button_, &QPushButton::clicked, this,
          &MainWindow::OnCaveGenerateClicked);
  connect(cave_import_button_, &QPushButton::clicked, this,
          &MainWindow::OnCaveImportClicked);
  connect(cave_export_button_, &QPushButton::clicked, this,
          &MainWindow::OnCaveExportClicked);
  connect(cave_step_button_, &QPushButton::clicked, this,
          &MainWindow::OnCaveStepClicked);
  connect(cave_auto_button_, &QPushButton::clicked, this,
          &MainWindow::OnCaveAutoClicked);
  connect(cave_stop_button_, &QPushButton::clicked, this,
          &MainWindow::OnCaveStopClicked);
}
...
void MainWindow::SetupQTab() {
  ...
  connect(q_training_timer_, &QTimer::timeout, this,
          &MainWindow::OnQTrainingTimerTimeout);
  ...
  connect(q_import_button_, &QPushButton::clicked, this,
          &MainWindow::OnQImportClicked);
  connect(q_train_button_, &QPushButton::clicked, this,
          &MainWindow::OnQTrainClicked);
  connect(q_run_button_, &QPushButton::clicked, this,
          &MainWindow::OnQRunClicked);
  connect(q_stop_button_, &QPushButton::clicked, this,
          &MainWindow::OnQStopClicked);
}
...
void MainWindow::OnQTrainClicked() {
  if (rl_maze_.IsEmpty()) {
    QMessageBox::information(this, "Информация", "Сначала загрузите лабиринт.");
    return;
  }

  std::pair<int, int> goal = {q_goal_row_spin_->value(),
...
void MainWindow::OnQTrainingTimerTimeout() {
  if (q_epochs_done_ >= q_total_epochs_target_) {
    ...
    q_info_label_->setText(
        QString("Training completed: %1 epochs.").arg(q_epochs_done_));
    return;
  }
...
void MainWindow::OnQRunClicked() {
  ...
  std::pair<int, int> start = {q_start_row_spin_->value(),
                               q_start_col_spin_->value()};
...
void MainWindow::OnSolveClicked() {
  ...
  if (start_row < 0 || start_row >= maze_.GetRows() || start_col < 0 ||
      start_col >= maze_.GetCols() || finish_row < 0 ||
      finish_row >= maze_.GetRows() || finish_col < 0 ||
      finish_col >= maze_.GetCols()) {
    QMessageBox::warning(
        this, "Ошибка",
...
void MainWindow::OnCaveExportClicked() {
  ...
  if (!cave_.SaveToFile(path.toStdString())) {
    QMessageBox::warning(this, "Ошибка", "Не удалось сохранить пещеру в файл.");
  }
}
...
void MainWindow::OnCaveStopClicked() { cave_timer_->stop(); }
