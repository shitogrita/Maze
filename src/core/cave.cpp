...
int Cave::CountAliveNeighbours(int row, int col) const {
  ...
  for (int row_offset = -1; row_offset <= 1; ++row_offset) {
    for (int col_offset = -1; col_offset <= 1; ++col_offset) {
      ...
      if (neighbour_row < 0 || neighbour_row >= rows_ || neighbour_col < 0 ||
          neighbour_col >= cols_) {
...
const std::vector<std::vector<int>>& Cave::GetCells() const { return cells_; }

bool Cave::IsEmpty() const { return rows_ == 0 || cols_ == 0; }
