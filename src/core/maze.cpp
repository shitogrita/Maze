...
void Maze::ValidateBottomWalls(std::vector<int>& maze, int row) {
  for (int i = 0; i < cols_; ++i) {
    if (CalculateBottomWalls(maze, row, maze[i]) ==
        CalculateSetMembers(maze, maze[i])) {
...
std::vector<std::pair<int, int> > Maze::GetNeighbours(int row,
                                                      int column) const {
...
bool Maze::IsEmpty() const { return rows_ == 0 || cols_ == 0; }

std::vector<std::pair<int, int> > Maze::Bfs(const std::pair<int, int> start,
  ...
  if (start.first < 0 || start.first >= rows_ || start.second < 0 ||
      start.second >= cols_) {
    return path;
  }

  if (finish.first < 0 || finish.first >= rows_ || finish.second < 0 ||
      finish.second >= cols_) {
    return path;
  }
  ...
  while (!index.empty()) {
    ...
    for (const auto& next : neighbours) {
      if (next.first < 0 || next.first >= rows_ || next.second < 0 ||
          next.second >= cols_) {
        continue;
      }
