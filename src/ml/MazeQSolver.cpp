...
void MazeQSolver::SetMaze(const Maze& maze) {
  ...
  q_values_.assign(maze_.GetRows(), std::vector<std::array<double, 4>>(
                                        maze_.GetCols(), {0.0, 0.0, 0.0, 0.0}));
}

void MazeQSolver::SetGoal(const std::pair<int, int>& goal) { goal_ = goal; }
...
std::pair<int, int> MazeQSolver::ApplyAction(int row, int col,
                                             int action) const {
  switch (action) {
    case kUp:
      return {row - 1, col};
...
int MazeQSolver::ChooseActionEpsilonGreedy(int row, int col,
...
void MazeQSolver::Train(int epochs, double alpha, double gamma,
                        double epsilon) {
  if (!IsReady()) {
    return;
  }
